        ...
        <div className='flex gap-2 relative'>
          ...
          <div className="relative" ref={dropdownRef}>
            <div className="flex items-center">
              <input
                type="text"
                className="w-36 h-[48px] rounded-2xl bg-white border-none  font-normal text-[20px] p-4 focus:outline-none cursor-pointer"
            ...
            {showDropdown && (
              <div className="fixed left-1/2 top-1/2 z-50 w-[900px] max-w-[98vw] bg-[#f3f8fa] rounded-2xl px-0 pt-3 pb-6 mx-auto shadow-lg font-inter transform -translate-x-1/2 -translate-y-1/2">
                <div className="flex items-center justify-between px-8 pb-6">
                  ...
                  <input
                    className="max-w-xs w-[300px] px-2.5 py-2 rounded-lg border border-[#dbe2e8] bg-white text-[16px] text-[#7d97ad] placeholder-[#b0c4d9] outline-none"
                      ...
                      {col.map((group) => (
                        <React.Fragment key={group.letter}>
                          {group.states.map(([name, ads], si) => (
                            <div
                              key={name}
                              className="flex items-center border-b border-[#e8edf2] bg-white hover:bg-[#e5eef4] transition px-1 py-[11px] cursor-pointer group"
                            >
                              {si === 0 && (
          ...
          <div className="relative w-110 mx-3.5">
            <input type="search" name="search" id="search" placeholder='I am looking for...' value={searchTerm}
        onChange={(e) => setSearchTerm(e.target.value)} className='w-[432px] h-[48px] rounded-2xl bg-white border-none  font-normal text-[20px] p-4 focus:outline-none'/>
